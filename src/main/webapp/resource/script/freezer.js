...
$(document).ready(function(){
	...
	$.ajax({
		...
		success : function(e){
			...
			$("input[name='spec']").val(e.spec);
			$("input[name='volume']").val(e.volume);
			$("input[name='area']").val(e.area);
			$("input[name='fan']").val(e.fan);
			$("input[name='mainFan']").val(e.mainFan);
			$("input[name='prepareFan']").val(e.prepareFan);
			$("input[name='passageway']").val(e.passageway);
			$("input[name='storageRack']").val(e.storageRack);
			$("input[name='monitor']").val(e.monitor);
			$("input[name='manufacturer']").val(e.manufacturer);
			if(e.imageUrl){
				//取消图片点击事件
				$baseInfoImg.unbind("click");
				//加载图片
				$("#uploadImg").css('display', 'none');
				$baseInfoImg.css('background-image', 'url('+e.imageUrl+')');
				//显示重新上传图片按钮
	...
	$.ajax({
		...
		success : function(data) {
			schemeDrop=data;
			//加载表格数据
			grid.load({enterpriseId:enterpriseId,schemeId:schemeId});
		},
		error : function(e) {
			alert("请求数据失败！status："+e.status);
		}
...
function schemeChange(e){
	grid.load({enterpriseId:enterpriseId,schemeId:schemeId,pointId:e.value});
}
//添加验证对象及布点方案
function addRow(){
	mini.open({
