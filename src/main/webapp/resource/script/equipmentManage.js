...
function editMaster(recodeID){
	...
	mini.open({
	    ...
	    onload: function () {
	    	//如果是毫秒，则转换为字符串形式
	    	if(!isNaN(recode.addDate)){
				var value = new Date(Number(recode.addDate));
		        recode.addDate=mini.formatDate(value, 'yyyy-MM-dd');
			}
	    	//向表单传参
...
function editPoint(recodeID){
	...
	mini.open({
	    ...
	    onload: function () {
	    	//如果是毫秒，则转换为字符串形式
	    	if(!isNaN(recode.deadline)){
				var value = new Date(Number(recode.deadline));
		        recode.deadline=mini.formatDate(value, 'yyyy-MM-dd');
			}
	    	//向表单传参
	    	recode.enterpriseId=enterpriseId;
	    	var iframe = this.getIFrameEl();
